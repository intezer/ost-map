    ...
    dataReady = function() {//This is called as soon as data is loaded
		...
		a.iterNodes(
			function (b) { //This is where we populate the array used for the group select box
				...
				// alert(b.x);
				a.clusters[b.attr.attributes['type']] || (a.clusters[b.attr.attributes['type']] = []);
				a.clusters[b.attr.attributes['type']].push(b.id);//SAH: push id not label
				a.color_family[b.attr.attributes['type']] = b.color;
			}
		);
...
function showCluster(a) {
    var temp = sigInst.clusters[a];
	var b = temp.concat(sigInst.clusters['Library']);
    if (b && 0 < b.length) {
        showGroups(!1);
        sigInst.detail = !0;
        b.sort();
